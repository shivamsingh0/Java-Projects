...
public class Main {
    ...
    static class TaskD {
        ...
        public void solve(int testNumber, FastReader in, PrintWriter out) {
            ...
            int[] p = TaskD.DisjointSets.createSets(n);
            ans = new int[m];
            List<TaskD.E2>[] tree = TaskD.LUtils.genArrayList(n);
            List<TaskD.E2>[] aa = TaskD.LUtils.genArrayList(n);
            boolean[] used = new boolean[m];
            List<Integer>[] trr = TaskD.LUtils.genArrayList(n);
            for (int i = 0; i < m; i++) {
                if (TaskD.DisjointSets.unite(p, e[i].from, e[i].to)) {
        ...
        static class SegmentTree {
            ...
            public SegmentTree(int n) {
                ...
                delta = new int[2 * n];
                ArrayUtils.fill(delta, getNeutralDelta());
            }
